...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local= 'on', gm_pool = 'on', arch='resnet50'):
        ...
        if self.non_local =='on':
            ...
            self.NL_2_idx = sorted([layers[1] - (i + 1) for i in range(non_layers[1])]) # 2，3
            self.NL_3 = nn.ModuleList(
                [Non_local(1024) for i in range(non_layers[2])])
            self.NL_3_idx = sorted([layers[2] - (i + 1) for i in range(non_layers[2])])#3，4，5
    ...
    def forward(self, x1, x2, modal=0, x1_g=None, x2_g=None):
        if modal == 0:
            ori_x = torch.cat((x1, x2), 0)
            x1 = self.visible_module(x1)
            x2 = self.thermal_module(x2)
            x_g = torch.cat((x1_g,x2_g),0)
            x = torch.cat((x1, x2), 0)
            with torch.no_grad():
                self.visualize(x,ori_x)
            pef_loss = self.PEF(x, x_g)
        elif modal == 1:
        ...
        if self.training:
            return x_pool, feat, self.classifier(feat), pef_loss
        else:
            return self.l2norm(x_pool), self.l2norm(feat)

    def visualize(self,featmap, img):
        select_id = [0,32,4,36,8,40,12,44] #[0,32,4,36,8,40,12,44] [16,48,20,52,24,56,28,60]
        b,c,h,w = featmap.shape[0],featmap.shape[1],featmap.shape[2],featmap.shape[3]
        featmap = featmap.data.cpu().numpy()
        img = img.data.cpu().numpy()
        for i in select_id:
            im = img[i]
            im = im.reshape(-1,144 * 288)
            im = im - np.min(im, axis = 1,keepdims=True)
            im = im / np.max(im, axis = 1,keepdims=True)
            im = im.reshape(3,288, 144).swapaxes(0,1).swapaxes(1,2)
            ori_img = np.uint8(255 * im)
            path = './heatmap/' + str(i)
            if not os.path.exists(path):
                os.mkdir(path)
            #ori_img = cv2.cvtColor(ori_img, cv2.COLOR_BGR2GRAY)
            cv2.imwrite(path + '/' + 'ori.jpg', ori_img)
            for id in range(0,c,4):
