...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=[320,160], patch_size=[4], in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=[320,160], patch_size=[4], in_chans=3, num_classes=1000,
...
class crossformer_b(nn.Module):
    def __init__(self, opt, class_num):
        super(crossformer_b, self).__init__()
        self.network = CrossFormer(img_size=224,depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24], group_size=[7, 7, 7, 7],
                                   patch_size=[4, 8, 16, 32], merge_size=[[2, 4], [2, 4], [2, 4]])
        model = torch.load("./crossformer-b.pth")['model']
        #model_dict = self.network.state_dict()
        #state_dict = {k: v for k, v in model.items() if "attn.biases" not in k and "attn.relative_position_index" not in k}
        #model_dict.update(state_dict)
        self.network.load_state_dict(model)
        self.network.head = nn.Linear(self.network.num_features,class_num)
        self.bottleneck = nn.BatchNorm1d(self.network.num_features)
        self.visible = nn.Sequential(self.network.patch_embed,self.network.pos_drop)
        self.thermal = copy.deepcopy(self.visible)
        self.bottleneck.apply(weights_init_kaiming)
        self.network.head.apply(weights_init_classifier)

    def forward(self, input1, input2,modal=0):
        if modal == 0:
            x1 = self.visible(input1)
            x2 = self.thermal(input2)
            x = torch.cat((x1, x2), 0)
        elif modal == 1:
            x = self.visible(input1)
        elif modal == 2:
            x = self.thermal(input2)
        for layer in self.network.layers:
            x = layer(x)
        x = self.network.norm(x)  # B L C
        x = self.network.avgpool(x.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        #feat = self.bottleneck(x)
        if self.training:
            return x,self.network.head(x)
        else:
            return nn.functional.normalize(x, dim=-1), nn.functional.normalize(x, dim=-1)
